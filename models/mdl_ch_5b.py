...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.feature_extractor = FeatureExtractor(ksize =cfg.feat_init_ksize, out_dim=cfg.feat_dim, conv_ch = 3)
        
        rotary_emb = LlamaRotaryEmbedding(cfg.feat_dim // 4, max_position_embeddings=self.cfg.max_times)
        ...
        self.mha1 = nn.MultiheadAttention(self.feat_dim, 4, batch_first = True)
        self.mha2 = nn.MultiheadAttention(self.mha1.kdim, 4, batch_first = True)
        self.head = LinearHead(self.mha1.kdim + self.backbone.config.hidden_size, 1)
...
#         out0 = torch.cat((feat_out, x), -1)
        ...
        logits = self.head(torch.cat([x[:,0],out2[:,0]],dim=-1)) # torch.Size([4, 1656, 9])       
#         preds = logits.softmax(1)
        output = {'idx': batch['idx']} 
        if 'target' in batch.keys():

            logits_aux1 = self.aux_head1(x[:,0]) # torch.Size([4, 1656, 9])    
            logits_aux2 = self.aux_head2(out2[:,0]) # torch.Size([4, 1656, 9])  
