...
class FeatureExtractor(nn.Module):
    def __init__(self, ksize = 7, out_dim = 128, conv_ch = 3, stride=4):
        super().__init__()   
        
        self.stride = stride
        self.stem_bn = nn.BatchNorm1d(out_dim, momentum=0.95)
        self.conv_stem = nn.Conv1d(conv_ch, out_dim, kernel_size=ksize, stride=stride, padding=ksize//2, bias=False)
        self.act = nn.ReLU()
    ...
    def forward(self, data, mask):
        ...
        mask = mask[:,::self.stride].to(torch.bool).clone()
...
#             self.backbone.gradient_checkpointing_enable()
        ...
        self.feature_extractor = FeatureExtractor(ksize =cfg.stem_kernel, out_dim=self.feat_dim, conv_ch = cfg.feat_input_dim, stride=cfg.stem_stride)
        
        rotary_emb = LlamaRotaryEmbedding(cfg.feat_dim // 4, max_position_embeddings=cfg.max_length // cfg.stem_stride)
        ...
        self.lstm = torch.nn.LSTM(self.feat_dim, self.feat_dim//2, num_layers=1, bias=True, 
                                         batch_first=True, bidirectional=True)
        self.head = LinearHead(self.feat_dim//2, 1)
...
#                           attention_mask=batch['attention_mask']).last_hidden_state
        ...
        feat_enc, mask = self.feature_extractor(batch['input'].permute(0,2,1), batch['attention_mask'].bool())
        feat_out = self.mha0(feat_enc, self.cos, self.sin, mask.bool())
        feat_out = self.mha1(feat_out, self.cos, self.sin, mask.bool())
        feat_out = self.mha2(feat_out, self.cos, self.sin, mask.bool())
        
        x = self.lstm(feat_out)[0]
        x = x.reshape(x.shape[0],x.shape[1],2,x.shape[2]//2).mean(2)
...
#         x = (feat_out * batch['attention_mask'][:,:,None]).sum(1) /  batch['attention_mask'][:,:,None].sum(1)
        x = mean_pooling(x, mask)
        logits = self.head(x) # torch.Size([4, 1656, 9])       
