...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.cos = torch.nn.parameter.Parameter(rotary_emb.cos_cached, requires_grad=False)[None,None,:]#[:, :, :seq_len, ...]#.to(dtype=x.dtype)
        self.sin = torch.nn.parameter.Parameter(rotary_emb.sin_cached, requires_grad=False)[None,None,:]#[:, :, :seq_len, ...]#.to(dtype=x.dtype)
...
#         self.loss_fn = nn.BCEWithLogitsLoss()
    ...
    def forward(self, batch):
        ...
        feat_out = self.mha0(feat_enc, self.cos.to(x.device), self.sin.to(x.device), batch['attention_mask'].bool())
