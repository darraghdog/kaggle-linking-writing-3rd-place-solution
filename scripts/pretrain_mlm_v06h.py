...
texts = texts.str.replace(" "*4, 'TAB ')
for L in string.ascii_lowercase:
    texts = texts.str.replace(L, 'i')
for L in range(10):
    texts = texts.str.replace(str(L), 'i')
...
dataset = PretrainingDataset(
    texts=[t for i,t in enumerate(texts) if i%40!=0], 
    tokenizer=tokenizer, 
...
training_args = TrainingArguments(
    output_dir=f"weights/pretrain_mlm_v06h/{args.model}",
    fp16=True,
