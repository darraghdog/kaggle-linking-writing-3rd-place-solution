...
arg('--n_chunks', type=int, default=1)
arg('--epochs', type=int, default=2)
arg('--accum', type=int, default=2)
arg('--maxlength', type=int, default=1024)
arg('--model', type=str, default='microsoft/deberta-v3-base')
arg('--outdir', type=str, default='/mount/lwp/models/pretrain_mlm_v06k_ch2d')
arg('--learning_rate', type=float, default=5e-5)
arg('--replace_char', type=str, default='X')
...
import pickle
with open('/mount/lwp/data/opentexts_X_16.p', 'rb') as fp:
...
tokenizer = AutoTokenizer.from_pretrained('/mount/lwp/notebooks/dev/deberta_spm_v4/')
...
cfg.update({'vocab_size':tokenizer.vocab_size})
...
training_args = TrainingArguments(
    output_dir=f"{args.outdir}/{args.model}",
    fp16=True,
