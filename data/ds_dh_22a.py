...
df = pd.read_csv(cfg.train_df + '.gz')
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        procd = {iddx: getEssays2(self.df.loc[iddx].copy())[0] for iddx in tqdm(self.df.index.unique())}
        procd = {k:v.replace('q', self.cfg.replace_char)\
                 .replace('\n', self.cfg.break_char) for k,v in procd.items()}
        procd = {k:v.replace(' '*4, self.cfg.tab_token) for k,v in procd.items()}
        dd = pd.Series(procd).reset_index()
        dd.columns = ['id', 'essay']
        dd.to_csv('datamount/processed_essays.csv.gz', index = False)
        '''
        ...
        reconstructed = reconstructed.replace(' '*4, self.cfg.tab_token)
        ...
        '''
        pd.Series(cumtimes).plot()
        pd.Series(uptimes).plot()
        for i,j in zip(reconstructed, uptimes, ): print(i, j//1000)
        '''
        ...
        feature_dict = self.tokenizer(reconstructed, 
                                      ...
                                      return_tensors = "pt")
        '''
        data = feature_dict.input_ids
        print(self.tokenizer.convert_ids_to_tokens(feature_dict.input_ids[0]))
        self.tokenizer.decode(feature_dict.input_ids[0])
        '''
        ...
        if self.aug:
            ...
            # pd.Series(augfeatfn(uptimes).numpy()).plot()
